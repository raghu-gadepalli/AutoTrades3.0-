-- AutoTrades historical archive for diagnostic stock movement rankings.
--
-- stock_rank remains the current operational-day table.  Completed rows are
-- copied here before stock_rank is cleared by the intraday initializer.
-- Archiving is idempotent by (symbol, rank_time).

CREATE TABLE IF NOT EXISTS `stock_rank_history` (
  `history_id` bigint NOT NULL AUTO_INCREMENT,
  `stock_rank_id` bigint NOT NULL,
  ...
  UNIQUE KEY `uq_stock_rank_history_symbol_time` (`symbol`,`rank_time`),
  KEY `idx_stock_rank_history_live_id` (`stock_rank_id`),
  KEY `idx_stock_rank_history_run` (`run_id`),
  KEY `idx_stock_rank_history_time_position` (`rank_time`,`rank_position`),
  KEY `idx_stock_rank_history_day_symbol` (`trading_day`,`symbol`),
  KEY `idx_stock_rank_history_day_class` (`trading_day`,`classification`),
  KEY `idx_stock_rank_history_day_tier` (`trading_day`,`attention_tier`),
  KEY `idx_stock_rank_history_day_score` (`trading_day`,`total_score`),
  KEY `idx_stock_rank_history_archived_on` (`archived_on`)
